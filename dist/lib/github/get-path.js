...
function getPath({ token = github.token, owner, repo, ref, path = '', }) {
    ...
    return cacheFallback({
        ...
        request: async () => {
            ...
            const res = await githubApi(`${reqPath}?${query}`, Object.assign({}, opts));
            let { body } = res;
            ...
            if (res.statusCode !== 200) {
                ...
                if (isTooLarge) {
                    ...
                    const dirRes = await githubApi(`${dirPath}?${query}`, Object.assign({}, opts)).json();
